...
bot.command('test', async (ctx) => {
  ...
  const daysBefore = process.env.REMIND_DAYS_BEFORE || 5;
  await ctx.reply(
    `Готово. Если выше ничего не появилось — значит ни у кого в /list нет дня рождения сегодня или через ${daysBefore} дней (с сегодняшним годом это не связано, проверяются только число и месяц).`
  );
});
